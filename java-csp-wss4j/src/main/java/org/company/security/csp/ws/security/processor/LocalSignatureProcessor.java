 ...
 */
package org.company.security.csp.ws.security.processor;
...
import org.company.security.csp.ws.security.components.crypto.LocalAlgorithmSuiteValidator;
import org.company.security.csp.ws.security.message.LocalWSSecSignature;
...
public class LocalSignatureProcessor implements Processor {
	...
	private static org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(LocalWSSecSignature.class); 
	...
	private void init(Crypto crypto) {
		String providerId = null;
		if(crypto != null) {
			String cryptoProvider = crypto.getCryptoProvider();
			if(CSP_PROVIDER.equals(cryptoProvider))
				providerId = CSPXML_PROVIDER;
		}
	...
	public List<WSSecurityEngineResult> handleToken(
		...
		if (algorithmSuite != null) {
			AlgorithmSuiteValidator algorithmSuiteValidator = new
					LocalAlgorithmSuiteValidator(algorithmSuite);
	...
	private XMLSignature verifyXMLSignature(
		...
		try {
			...
			Crypto crypto = wsDocInfo.getCrypto();
			if(crypto != null) {
				Provider p = Security.getProvider(crypto.getCryptoProvider());
				if(p != null) {
					((DOMValidateContext)context).setProperty("org.jcp.xml.dsig.internal.dom.SignatureProvider", p);
				}
			}
